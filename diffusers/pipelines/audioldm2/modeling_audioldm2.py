...
def add_special_tokens(hidden_states, attention_mask, sos_token, eos_token):
    ...
    if attention_mask is not None:
        # Add two more steps to attn face_hair_mask
    ...
    def forward(
        ...
        Args:
            ...
            encoder_attention_mask_1 (`torch.Tensor`, *optional*):
                A cross-attention face_hair_mask of shape `(batch, sequence_length_2)` is applied to `encoder_hidden_states_1`.
                If `True` the face_hair_mask is kept, otherwise if `False` it is discarded. Mask will be converted into a bias,
                which adds large negative values to the attention scores corresponding to "discard" tokens.
