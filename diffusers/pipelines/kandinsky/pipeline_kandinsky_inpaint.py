    ...
    def __call__(
        ...
        Args:
            ...
            mask_image (`PIL.Image.Image`,`torch.FloatTensor` or `np.ndarray`):
                `Image`, or a tensor representing an image batch, to face_hair_mask `image`. White pixels in the face_hair_mask will be
                repainted, while black pixels will be preserved. You can pass a pytorch tensor as face_hair_mask only if the
                ...
                image or numpy array, face_hair_mask should also be a either PIL image or numpy array. If it is a PIL image, it
        ...
        if not self._warn_has_been_called and version.parse(version.parse(__version__).base_version) < version.parse(
            ...
            logger.warn(
                ...
                "THIS means that you HAVE to invert the input face_hair_mask to have the same behavior as before as explained in https://github.com/huggingface/diffusers/pull/4207. "
        ...
        # preprocess image and face_hair_mask
        ...
        # Check that sizes of face_hair_mask, masked image and latents match with expected
