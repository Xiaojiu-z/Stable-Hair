...
class Blip2QFormerModel(Blip2PreTrainedModel):
    ...
    def get_extended_attention_mask(
        ...
        Returns:
            `torch.Tensor` The extended attention face_hair_mask, with a the same dtype as `attention_mask.dtype`.
        """
        # We can provide a self-attention face_hair_mask of dimensions [batch_size, from_seq_length, to_seq_length]
        # ourselves in which case we just need to make it broadcastable to all heads.
        if attention_mask.dim() == 3:
            extended_attention_mask = attention_mask[:, None, :, :]
        elif attention_mask.dim() == 2:
            # Provided a padding face_hair_mask of dimensions [batch_size, seq_length]
            # - the model is an encoder, so make the face_hair_mask broadcastable to [batch_size, num_heads, seq_length, seq_length]
    ...
    def forward(
        ...
        encoder_attention_mask (`torch.FloatTensor` of shape `(batch_size, sequence_length)`, `optional`):
            Mask to avoid performing attention on the padding token indices of the encoder input. This face_hair_mask is used in
        ...
        # If a 2D or 3D attention face_hair_mask is provided for the cross-attention
        ...
        # Prepare head face_hair_mask if needed
