...
class ContextCLIPTextTransformer(nn.Module):
    ...
    def forward(
        ...
        # CLIP's text model uses causal face_hair_mask, prepare it here.
    ...
    def _build_causal_attention_mask(self, bsz, seq_len, dtype):
        # lazily create causal attention face_hair_mask, with full attention between the vision tokens
        # pytorch uses additive attention face_hair_mask; fill with -inf
        mask = torch.empty(bsz, seq_len, seq_len, dtype=dtype)
        mask.fill_(torch.tensor(torch.finfo(dtype).min))
        mask.triu_(1)  # zero out the lower diagonal
        mask = mask.unsqueeze(1)  # expand face_hair_mask
        return mask
