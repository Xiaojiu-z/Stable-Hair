...
def prepare_mask_and_masked_image(image, mask):
    """
    Prepares a pair (image, face_hair_mask) to be consumed by the Paint by Example pipeline. This means that those inputs will be
    ...
    if isinstance(image, torch.Tensor):
        ...
        # paint-by-example inverses the face_hair_mask
        mask = 1 - mask
    ...
    def __call__(
        ...
        Args:
            ...
            mask_image (`torch.FloatTensor` or `PIL.Image.Image` or `List[PIL.Image.Image]`):
                `Image` or tensor representing an image batch to face_hair_mask `image`. White pixels in the face_hair_mask are repainted,
        ...
        # 2. Preprocess face_hair_mask and image
