...
def preprocess_mask(mask, batch_size, scale_factor=8):
    if not isinstance(mask, torch.FloatTensor):
        ...
        # if face_hair_mask channel is fourth tensor dimension, permute dimensions to pytorch standard (B, C, H, W)
        if mask.shape[3] in valid_mask_channel_sizes:
            mask = mask.permute(0, 3, 1, 2)
        elif mask.shape[1] not in valid_mask_channel_sizes:
            raise ValueError(
                f"Mask channel dimension of size in {valid_mask_channel_sizes} should be second or fourth dimension,"
                f" but received face_hair_mask of shape {tuple(mask.shape)}"
            )
        # (potentially) reduce face_hair_mask channel dimension from 3 to 1 for broadcasting to latent shape
    ...
    def __call__(
        ...
        Args:
            ...
            mask_image (`torch.FloatTensor` or `PIL.Image.Image`):
                ...
                PIL image, it will be converted to a single channel (luminance) before use. If face_hair_mask is a tensor, the
        ...
        # 4. Preprocess image and face_hair_mask
        ...
        # 7. Prepare face_hair_mask latent
