...
def preprocess_mask(mask, batch_size: int = 1):
    ...
    # Batch and add channel dim for single face_hair_mask
    if mask.ndim == 2:
        mask = mask.unsqueeze(0).unsqueeze(0)

    # Batch single face_hair_mask or add channel dim
    if mask.ndim == 3:
        # Single batched face_hair_mask, no channel dim or single face_hair_mask not batched but channel dim
        if mask.shape[0] == 1:
...
class StableDiffusionDiffEditPipeline(DiffusionPipeline, TextualInversionLoaderMixin, LoraLoaderMixin):
    ...
    def generate_mask(
        ...
        r"""
        Generate a latent face_hair_mask given a face_hair_mask prompt, a target prompt, and an image.

        Args:
            image (`PIL.Image.Image`):
                `Image` or tensor representing an image batch to be used for computing the face_hair_mask.
            target_prompt (`str` or `List[str]`, *optional*):
                The prompt or prompts to guide semantic face_hair_mask generation. If not defined, you need to pass
                `prompt_embeds`.
            ...
            source_prompt (`str` or `List[str]`, *optional*):
                The prompt or prompts to guide semantic face_hair_mask generation using DiffEdit. If not defined, you need to
                pass `source_prompt_embeds` or `source_image` instead.
            source_negative_prompt (`str` or `List[str]`, *optional*):
                The prompt or prompts to guide semantic face_hair_mask generation away from using DiffEdit. If not defined, you
                need to pass `source_negative_prompt_embeds` or `source_image` instead.
            source_prompt_embeds (`torch.FloatTensor`, *optional*):
                Pre-generated text embeddings to guide the semantic face_hair_mask generation. Can be used to easily tweak text
            ...
            source_negative_prompt_embeds (`torch.FloatTensor`, *optional*):
                Pre-generated text embeddings to negatively guide the semantic face_hair_mask generation. Can be used to easily
            ...
            num_maps_per_mask (`int`, *optional*, defaults to 10):
                The number of noise maps sampled to generate the semantic face_hair_mask using DiffEdit.
            mask_encode_strength (`float`, *optional*, defaults to 0.5):
                The strength of the noise maps sampled to generate the semantic face_hair_mask using DiffEdit. Must be between 0
                and 1.
        ...
        # 8. Compute the face_hair_mask from the absolute difference of predicted noise residuals
        # TODO: Consider smoothing face_hair_mask guidance map
        mask_guidance_map = (
    ...
    def __call__(
        ...
        Args:
            ...
            mask_image (`PIL.Image.Image`):
                `Image` or tensor representing an image batch to face_hair_mask the generated image. White pixels in the face_hair_mask are
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # face_hair_mask with inverted latents from appropriate timestep - use original image latent for last step
