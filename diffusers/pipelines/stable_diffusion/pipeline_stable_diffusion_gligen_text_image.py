...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        ...         "dragon",
        ...         "placeholder",
        ...     ],  # Can use any text instead of `placeholder` token, because we will use face_hair_mask here
        ...     gligen_images=[
        ...         gligen_placeholder,
        ...         gligen_image,
        ...     ],  # Can use any image in gligen_placeholder, because we will use face_hair_mask here
...
class StableDiffusionGLIGENTextImagePipeline(DiffusionPipeline):
    ...
    def draw_inpaint_mask_from_boxes(self, boxes, size):
        """
        Create an inpainting face_hair_mask based on given boxes. This function generates an inpainting face_hair_mask using the provided
    ...
    def complete_mask(self, has_mask, max_objs, device):
        """
        Based on the input face_hair_mask corresponding value `0 or 1` for each phrases and image, face_hair_mask the features
    ...
    def get_cross_attention_kwargs_with_grounded(
        ...
        """
        Prepare the cross-attention kwargs containing information about the grounded input (boxes, face_hair_mask, image
    ...
    def get_cross_attention_kwargs_without_grounded(self, hidden_size, repeat_batch, max_objs, device):
        """
        Prepare the cross-attention kwargs without information about the grounded input (boxes, face_hair_mask, image embedding,
    ...
    def __call__(
        ...
        Args:
            ...
            input_phrases_mask (`int` or `List[int]`):
                pre phrases face_hair_mask input defined by the correspongding `input_phrases_mask`
            input_images_mask (`int` or `List[int]`):
                pre images face_hair_mask input defined by the correspongding `input_images_mask`
        ...
        if gligen_inpaint_image is not None:
            ...
            # Generate an inpainting face_hair_mask
            ...
            # Convert a single face_hair_mask into a batch of masks with a batch size of 1
